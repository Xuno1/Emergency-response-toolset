...
def check_shell(content):
    try:
        ...
        # 下载执行类
                ...
                ' http' in content) and (
                ('php ' in content) or ('perl' in content) or ('python ' in content) or ('python3 ' in content) or ('sh ' in content) or (
                'bash ' in content)):
            return content
        return False
    except:
...
def logs_backup():
    SYS_PATH = get_value('SYS_PATH')
    bakfile = SYS_PATH + '/log/logs.tar.gz'
    try:
        tar_info = os.popen("tar -czvf " + bakfile +" /var/log/audit /var/log/auth.log* /var/log/lastlog  /var/log/wtmp /var/log/ufw.log  /var/log/secure* /var/log/cron* /var/log/btmp /var/log/firewalld* /var/log/audit 2>/dev/null").read().splitlines()
        for file in tar_info:
            print("已打包:" + file)
    except print(0):
        pass
