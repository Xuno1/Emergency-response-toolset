...
class Proc_Analysis:
    ...
    def work_analysis(self):
        ...
        try:
            ...
            p6 = Popen(
                "grep -viE 'systemd|rsyslogd|mysqld|redis|apache|nginx|mongodb|docker|memcached|tomcat|jboss|java|php|python'",
    ...
    def keyi_analysis(self):
        ...
        try:
            p1 = Popen("ps -efwww 2>/dev/null", stdout=PIPE, shell=True)
            p2 = Popen("grep -iE 'minerd|r00t|sqlmap|nmap|hydra|aircrack|fscan|xray|frp|nps|npc|teamserver|sliver|hack|haiduc|scan|ew|proxy|.elf|neoreg|LinEnum'", stdin=p1.stdout, stdout=PIPE, shell=True)
            ...
            for pro in process:
                pro_info = pro.strip().split(' ', 3)
                #获取单个进程详细信息
                single_proc = Popen("ps -ef | grep " + pro_info[1] + " | grep -v 'grep' | awk '{$1=$2=$3=$4=$5=$6=$7=\"\";print  $0}'", stdin=p3.stdout, stdout=PIPE, shell=True).stdout.read().splitlines()
                pro_info[3] = single_proc[0].strip()
